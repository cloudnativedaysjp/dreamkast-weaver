...
func (s service_local_stub) Vote(ctx context.Context, a0 model.VoteInput) (err error) {
...
func (s service_local_stub) VoteCounts(ctx context.Context, a0 model.ConfName) (r0 []*model.VoteCount, err error) {
	span := trace.SpanFromContext(ctx)
	if span.SpanContext().IsValid() {
		// Create a child span for this method.
...
func (s service_client_stub) Vote(ctx context.Context, a0 model.VoteInput) (err error) {
	// Update metrics.
	start := time.Now()
...
func (s service_client_stub) VoteCounts(ctx context.Context, a0 model.ConfName) (r0 []*model.VoteCount, err error) {
	// Update metrics.
	start := time.Now()
	...
	// Decode the results.
	dec := codegen.NewDecoder(results)
	r0 = serviceweaver_dec_slice_ptr_VoteCount_2bfdc7b9(dec)
	err = dec.Error()
	return
}
...
func (s service_server_stub) voteCounts(ctx context.Context, args []byte) (res []byte, err error) {
	...
	*(*string)(&a0) = dec.String()
	...
	// Encode the results.
	enc := codegen.NewEncoder()
	serviceweaver_enc_slice_ptr_VoteCount_2bfdc7b9(enc, r0)
	enc.Error(appErr)
	return enc.Data(), nil
}
...
func serviceweaver_enc_ptr_VoteCount_2875672f(enc *codegen.Encoder, arg *model.VoteCount) {
	if arg == nil {
		enc.Bool(false)
	} else {
		enc.Bool(true)
...
func serviceweaver_dec_ptr_VoteCount_2875672f(dec *codegen.Decoder) *model.VoteCount {
	if !dec.Bool() {
		return nil
	}
	var res model.VoteCount
	(&res).WeaverUnmarshal(dec)
	return &res
}

func serviceweaver_enc_slice_ptr_VoteCount_2bfdc7b9(enc *codegen.Encoder, arg []*model.VoteCount) {
	if arg == nil {
		enc.Len(-1)
		return
	}
	enc.Len(len(arg))
	for i := 0; i < len(arg); i++ {
		serviceweaver_enc_ptr_VoteCount_2875672f(enc, arg[i])
	}
}

func serviceweaver_dec_slice_ptr_VoteCount_2bfdc7b9(dec *codegen.Decoder) []*model.VoteCount {
	n := dec.Len()
	if n == -1 {
		return nil
	}
	res := make([]*model.VoteCount, n)
	for i := 0; i < n; i++ {
		res[i] = serviceweaver_dec_ptr_VoteCount_2875672f(dec)
	}
	return res
}
