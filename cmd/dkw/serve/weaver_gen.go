...
func init() {
	codegen.Register(codegen.Registration{
		...
		RefData: "⟦b6c47d6e:wEaVeReDgE:github.com/ServiceWeaver/weaver/Main→dreamkast-weaver/internal/graph/Resolver⟧\n",
	})
}
...
func (s main_local_stub) Main(ctx context.Context) (err error) {
	...
	if span.SpanContext().IsValid() {
		...
		ctx, span = s.tracer.Start(ctx, "serve.Main.Main", trace.WithSpanKind(trace.SpanKindInternal))
		defer func() {
			if err != nil {
...
func (s main_client_stub) Main(ctx context.Context) (err error) {
	...
	if span.SpanContext().IsValid() {
		// Create a child span for this method.
		ctx, span = s.stub.Tracer().Start(ctx, "serve.Main.Main", trace.WithSpanKind(trace.SpanKindClient))
	}
